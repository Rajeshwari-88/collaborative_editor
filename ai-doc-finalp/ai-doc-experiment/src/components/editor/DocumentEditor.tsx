...
const DocumentEditor: React.FC = () => {
  ...
  const { showVideoCallToast, showSuccessToast, showErrorToast, showInfoToast } = useToast();
  ...
  useEffect(() => {
    ...
    if (id) {
      ...
      if (socket) {
        ...
        // Global comment notification for users even if Comments panel is closed
        socket.on("comment-added", (comment: any) => {
          if (!comment || !comment.user) return;
          if (comment.user.id === user?.id) return;
          const panelOpen = showSidebar && sidebarContent === "comments";
          if (!panelOpen) {
            ...
            if (typeof window !== 'undefined' && 'Notification' in window) {
              const notify = () => {
                try {
                  const n = new Notification('New Comment', {
                    body: `${name} commented on this document`,
                    icon: '/favicon.ico'
                  });
                  n.onclick = () => {
                    window.focus();
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        await fetch(`${API_URL}/api/documents/${id}/versions`, {
          method: "POST",
  ...
  const triggerBlockGrammarCheck = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const resp = await fetch(`${API_URL}/api/ai/grammar-check`, {
        method: "POST",
        headers: {
  ...
  const saveDocument = async (contentToSave?: string) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${API_URL}/api/documents/${documentData.id}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const triggerInlineSuggestion = async (currentContent: string) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_URL}/api/ai/complete`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  return (
    <div className="min-h-screen relative bg-gradient-to-br from-teal-50 via-white to-emerald-50 flex">
      ...
      <div className="flex-1 flex flex-col">
        {/* Header */}
        <header className="px-6 py-4 border-b border-gray-200 bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60">
                ...
                <div className="flex items-center text-sm text-gray-600">
                  ...
                  {saving && (
                    <>
                      ...
                      <span className="text-teal-700">Saving...</span>
                    </>
                  )}
                  {lastSaved && !saving && (
                    <>
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="flex -space-x-2">
                {activeUsers.slice(0, 3).map((user) => (
                  <div
                    key={user.userId}
                    className="w-8 h-8 bg-teal-500 rounded-full flex items-center justify-center text-white text-sm font-medium border-2 border-white"
                    title={user.name}
                  >
        ...
        <div className="flex-1 overflow-hidden relative">
                ...
                <div className="flex items-center gap-2 mb-3">
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        const token = localStorage.getItem("token");
                        const resp = await fetch(`${API_URL}/api/ai/mom`, {
                          method: "POST",
                          headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
                          body: JSON.stringify({
                            documentId: documentData?.id,
                            transcript: (window as any)._lastCallTranscript || "",
                          }),
                        });
                        if (resp.ok) {
                          const data = await resp.json();
          ...
          {incomingCall && incomingModalOpen && (
            <div className="fixed inset-0 z-30 flex items-center justify-center">
              ...
              <div className="relative z-40 w-full max-w-sm component-shell component-padding text-center">
                <div className="w-16 h-16 mx-auto mb-3 rounded-full bg-teal-600 text-white flex items-center justify-center text-xl font-semibold">
          ...
          <div
            ...
            className="h-full p-6 focus:outline-none editor-content overflow-y-auto bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/50 rounded-xl shadow ring-1 ring-gray-200 mx-4 my-4"
            style={
              {
                minHeight: "100%",
      ...
      {showSidebar && (
        <>
          ...
          {/* Mobile drawer */}
          <div className="sm:hidden fixed right-0 top-0 h-full w-full max-w-[90%] bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/70 z-50 shadow-lg">
          ...
          {/* Desktop sidebar */}
          <div className="hidden sm:block w-80 border-l border-gray-200 bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60">
            {sidebarContent === "collaborators" && (
              <CollaboratorsList
                activeUsers={activeUsers}
                document={documentData}
